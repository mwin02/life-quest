...
if (fs.existsSync(envPath)) {
  ...
  console.warn(
    "⚠️  .env.local file not found. Looking for environment variables...\n",
  );
}
...
async function getTestToken() {
  ...
  if (error) {
    ...
    console.log(
      "💡 You can create a user in the Supabase Dashboard: Authentication > Users",
    );
    process.exit(1);
  }
...
curl http://localhost:3000/api/adventures \\
  ...
  console.log(
    `   (${Math.round((data.session.expires_at * 1000 - Date.now()) / 1000 / 60)} minutes from now)`,
  );
}
